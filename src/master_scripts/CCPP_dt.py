...
for path in [
    ...
    sys.path.append(
        os.path.join(
            ...
            path))
from datasets import get_CCPP_data
from models import get_decision_tree
...
opts = args['CCPP']
X_data, Y_data = get_CCPP_data(os.path.join(opts['path'], opts['file']))
...
if train_opts['parameter_tuning'] is True:  # tuning
    ...
    model = get_decision_tree(args['decision_tree'])
